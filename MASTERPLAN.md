...
- ✅ **7.7. — 2 von 5 Modellen jetzt freigeschaltet:** `minimax-m3` und `kimi-k2p7-code` funktionieren live (echt getestet). Die 3 Gemma-4-Varianten weiterhin 404 — Freischaltung läuft also gestaffelt, kein Problem im Code. `kimi-k2p7-code` ist im direkten Vergleich deutlich tokeneffizienter als `minimax-m3` (187–230 vs. 303–310 Tokens auf denselben 2 Testaufgaben) → jetzt als Default-Reihenfolge in `config.py` hinterlegt (`kimi-k2p7-code` zuerst).
