...
- ✅ **Exakte Modell-ID bestätigt** (fireworks.ai/models, öffentliche Katalogseite): `accounts/fireworks/models/gemma-4-26b-a4b-it` (mit Prefix — in `config.py` jetzt korrekt hinterlegt). Trotzdem weiterhin 404 bei echtem Call → das Modell ist im Katalog gelistet, aber auf Sebastians Account **noch nicht freigeschaltet**. Nächster Schritt: lablab.ai-Dashboard auf "Fireworks-Credits einlösen" prüfen, sonst im AMD-Discord-Support-Channel fragen, wann/wie die 5 Track-1-Modelle freigeschaltet werden. Für die Submission selbst kein Blocker (Harness injiziert eigenen Key mit Zugriff), nur lokales Testen ist betroffen.
